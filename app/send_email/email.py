...
from .email_catalog_OLD import EMAIL_TYPES
...
def send_email(recipient_name: str, recipient_email: str, email_type: str) -> None:
    ...
    **Parameters:**
        ...
        email_type (str): A key from EMAIL_TYPES dictionary (in email_catalogue.py)
    ```
    """
    email_body = render_template(
        EMAIL_TYPES[email_type]["template"],  # email template name
        name=recipient_name,
        content=EMAIL_TYPES[email_type]["content"]
    )
    new_email = EmailMessage(
        subject = f"{EMAIL_TYPES[email_type]["subject"]}",
